...
class App:
    ...
    def visualize_dfs_search(self):
            try:          
                ...
                messagebox.showerror("Error Visualisasi DFS", f"Kesalahan terjadi saat visualisasi DFS: {e}")
...
class DFSSearchVisualizationWindow(ctk.CTkToplevel):
    ...
    def destroy(self):
        # print(f"DFSSearchVisualizationWindow destroy called. Animation job: {self.animation_job}")
        if self.animation_job:
            self.after_cancel(self.animation_job)
            self.animation_job = None
        super().destroy()

    def _depth_slider_changed(self, value):
        self.max_depth_var.set(int(value))
        self.max_depth_label.configure(text=f"{int(value)}")
        self.calculate_node_positions() 
        self.draw_tree_structure()      
        self.update_visualization()
    ...
    def get_node_levels_and_children_status(self, root_node_obj): # Renamed param for clarity
        data = {} 
        queue = [(root_node_obj, 0)] 
        visited_nodes = set()
        # ... (rest of the method is identical to VisualizationWindow, ensure it's here)
        return data # Placeholder, ensure full method is copied if not already present
    ...
    def calculate_node_positions(self):
        # print ("Memulai calculate_node_positions DFSSearch")
        self.node_positions.clear()
        if not self.tree_root_node:
            return
        ...
        max_depth_limit = self.max_depth_var.get()
        leaf_x_counter = [50]

        def layout_recursive(node_obj, current_level):
            if not node_obj or current_level > max_depth_limit:
                return {'min_x': float('inf'), 'max_x': float('-inf')}

            has_children_if_at_max_depth = bool(node_obj.children) and current_level == max_depth_limit
            is_effective_leaf_for_layout = (not node_obj.children) or (current_level == max_depth_limit)

            if is_effective_leaf_for_layout:
                x = leaf_x_counter[0]
                self.node_positions[node_obj.name] = (x, current_level * y_spacing + 50, current_level, has_children_if_at_max_depth)
                leaf_x_counter[0] += (node_width + x_spacing_min)
                return {'min_x': x, 'max_x': x}

            min_child_x_extent = float('inf')
            max_child_x_extent = float('-inf')
            valid_child_layouts_exist = False

            for child_node_obj in node_obj.children:
                if child_node_obj:
                    if current_level + 1 <= max_depth_limit:
                        child_layout_info = layout_recursive(child_node_obj, current_level + 1)
                        if child_layout_info['min_x'] != float('inf'):
                            min_child_x_extent = min(min_child_x_extent, child_layout_info['min_x'])
                            max_child_x_extent = max(max_child_x_extent, child_layout_info['max_x'])
                            valid_child_layouts_exist = True
            
            if not valid_child_layouts_exist:
                x = leaf_x_counter[0]
                self.node_positions[node_obj.name] = (x, current_level * y_spacing + 50, current_level, has_children_if_at_max_depth)
                leaf_x_counter[0] += (node_width + x_spacing_min)
                return {'min_x': x, 'max_x': x}

            node_x = (min_child_x_extent + max_child_x_extent) / 2
            self.node_positions[node_obj.name] = (node_x, current_level * y_spacing + 50, current_level, False)
            
            return {'min_x': min_child_x_extent, 'max_x': max_child_x_extent}

        layout_recursive(self.tree_root_node, 0)
        # print(f"DFS Node positions after calc: {self.node_positions}")
    ...
    def draw_tree_structure(self):
        ...
        for node_name, pos_data in self.node_positions.items():
            x_node, y_node, level_node, has_children_indicator = pos_data
            
            node_obj = self.all_nodes.get(node_name)
            if not node_obj: continue

            if level_node < max_depth_limit:
                return
            ...
            for child in node.children:
                ...
                for child_obj in node_obj.children:
                    if child_obj and child_obj.name in self.node_positions:
                        child_pos_data_check = self.node_positions[child_obj.name]
                        if child_pos_data_check[2] <= max_depth_limit: # child_level is at index 2
                            x_child, y_child, _, _ = child_pos_data_check
                            
                            x1_draw_line = x_node * self.scale_factor
                            y1_draw_line_start = (y_node * self.scale_factor) + (node_height / 2 * self.scale_factor)
                            
                            x2_draw_line = x_child * self.scale_factor
                            y2_draw_line_end = (y_child * self.scale_factor) - (node_height / 2 * self.scale_factor)
                            
                            self.canvas.create_line(
                                x1_draw_line, y1_draw_line_start,
                                x2_draw_line, y2_draw_line_end,
                                fill="#666666", width=1.5
                            )

            x_scaled, y_scaled = x_node * self.scale_factor, y_node * self.scale_factor
            current_node_actual_width_scaled = max(node_width, len(node_name) * 8) * self.scale_factor
            height_scaled = node_height * self.scale_factor
            ...
            if has_children_indicator:
                display_text += " [+]"
            
            text_id = self.canvas.create_text(
                x_scaled, y_scaled,
                text=display_text, font=("Arial", int(10 * self.scale_factor)), fill="#333333", tags="node_text"
            )
